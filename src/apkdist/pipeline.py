...
def build_apk(android_root: str, module_name: str, build_variant: str) -> bool:
...
def main(argv=None):
    ...
    if loaded_env:
        ...
        print(f"ℹ️  No .env found in current/parent directories; tried global config at {user_config_dir()}")
    ...
    should_reuse_fresh_apk = bool(existing_apk) and (not args.force) and is_apk_fresh(existing_apk)

    if args.dry_run:
        print("🧪 Dry-run mode — no changes will be made\n")
        if should_reuse_fresh_apk:
            age_mins = int((time.time() - os.path.getmtime(existing_apk)) / 60)
            print(f"⏩ [DRY-RUN] Would reuse fresh APK ({age_mins}m old) without bumping: {existing_apk}")
            print("   Use --force to bump/build anyway.")
        else:
        ...
        if existing_apk:
            print(f"📦 [DRY-RUN] Found APK: {existing_apk}")
        else:
    ...
    if should_reuse_fresh_apk:
        ...
        version_to_send = current_version
        apk_path = existing_apk
    else:
        version_to_send = bump_version(version_file, args.type)
        if not build_apk(config.android_root, config.module_name, config.build_variant):
            sys.exit(1)
